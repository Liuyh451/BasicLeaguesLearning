...
# 过滤prediction类别为0数据集
predictions.filter(predictions['prediction'] == 0).select('Title', 'Category', 'probability', 'label', 'prediction').orderBy('probability', accending=False).show(n=10, truncate=30)
